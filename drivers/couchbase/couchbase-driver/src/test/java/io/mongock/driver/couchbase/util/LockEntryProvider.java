...
public class LockEntryProvider {
  ...
  private static final LockEntry LOCK_OWNER_1_NOT_EXPIRED = new
      LockEntry(LOCK_KEY, LockStatus.LOCK_HELD.name(), "owner-1", new Date(System.currentTimeMillis() + 180000));
  private static final LockEntry LOCK_OWNER_1_NOT_EXPIRED_UPDATED = new
      LockEntry(LOCK_KEY, LockStatus.LOCK_HELD.name(), "owner-1", new Date(System.currentTimeMillis() + 360000));
  private static final LockEntry LOCK_OWNER_1_EXPIRED = new
      LockEntry(LOCK_KEY, LockStatus.LOCK_HELD.name(), "owner-1", new Date(System.currentTimeMillis() - 10000));
  private static final LockEntry LOCK_OWNER_2_NOT_EXPIRED = new
      LockEntry(LOCK_KEY, LockStatus.LOCK_HELD.name(), "owner-2", new Date(System.currentTimeMillis() + 180000));
