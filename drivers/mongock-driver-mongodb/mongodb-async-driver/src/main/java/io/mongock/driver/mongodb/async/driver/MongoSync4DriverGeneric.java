...
import io.mongock.driver.mongodb.async.repository.MongoReactiveChangeEntryRepository;
import io.mongock.driver.mongodb.async.repository.MongoReactiveLockRepository;
import io.mongock.driver.mongodb.async.repository.ReadWriteConfiguration;
...
public abstract class MongoSync4DriverGeneric extends TransactionalConnectionDriverBase implements DriverLegaciable {
  ...
  protected LockRepository getLockRepository() {
    if (lockRepository == null) {
      ...
      lockRepository = new MongoReactiveLockRepository(collection, getReadWriteConfiguration());
  ...
  public ChangeEntryService getChangeEntryService() {
    if (changeEntryRepository == null) {
      changeEntryRepository = new MongoReactiveChangeEntryRepository(mongoDatabase.getCollection(getMigrationRepositoryName()), getReadWriteConfiguration());
